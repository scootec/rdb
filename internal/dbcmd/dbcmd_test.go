...
func TestDumpCmd(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			env:          map[string]string{},
			dbType:       "postgres",
			wantCmd:      []string{"pg_dumpall", "--clean", "--if-exists", "-U", "postgres"},
			wantExtraEnv: nil,
		},
		{
			...
			dbType:       "postgres",
			wantCmd:      []string{"pg_dumpall", "--clean", "--if-exists", "-U", "app"},
		...
		},
		{
			...
			dbType:       "mysql",
			wantCmd:      []string{"mysqldump", "--user=root", "--all-databases", "--single-transaction", "--quick", "--routines", "--events", "--triggers", "--add-drop-table"},
		...
		},
		{
			...
			dbType:       "mysql",
			wantCmd:      []string{"mysqldump", "--user=app", "--all-databases", "--single-transaction", "--quick", "--routines", "--events", "--triggers", "--add-drop-table"},
		...
		},
		{
			...
			env:          map[string]string{},
			dbType:       "mysql",
			wantCmd:      []string{"mysqldump", "--user=", "--all-databases", "--single-transaction", "--quick", "--routines", "--events", "--triggers", "--add-drop-table"},
			wantExtraEnv: nil,
		},
		{
			...
			dbType:       "mariadb",
			wantCmd:      []string{"mariadb-dump", "--user=root", "--all-databases", "--single-transaction", "--quick", "--routines", "--events", "--triggers", "--add-drop-table"},
			wantExtraEnv: nil,
		},
		{
		...
		},
		{
			...
			dbType:       "mariadb",
			wantCmd:      []string{"mariadb-dump", "--user=app", "--all-databases", "--single-transaction", "--quick", "--routines", "--events", "--triggers", "--add-drop-table"},
			wantExtraEnv: []string{"MYSQL_PWD=apppw"},
		},
		{
...
func TestImportCmd(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			env:          map[string]string{},
			dbType:       "postgres",
			wantCmd:      []string{"psql", "--set", "ON_ERROR_STOP=on", "-U", "postgres", "-d", "postgres"},
			wantExtraEnv: nil,
		},
		{
			name:         "postgres uses POSTGRES_USER and POSTGRES_PASSWORD",
			env:          map[string]string{"POSTGRES_USER": "app", "POSTGRES_PASSWORD": "s3cret"},
			dbType:       "postgres",
			wantCmd:      []string{"psql", "--set", "ON_ERROR_STOP=on", "-U", "app", "-d", "postgres"},
			wantExtraEnv: []string{"PGPASSWORD=s3cret"},
		},
		{
			name:         "mysql uses root with MYSQL_ROOT_PASSWORD",
